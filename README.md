...
Scan a handwritten **Support A Child** pledge card with your phone → Google Gemini reads it → you review → it saves to a Google Sheet. Every saved pledge shows up in the Records tab, straight from the sheet.

No server to run and no build step: `index.html` is a static page you host anywhere (e.g. GitHub Pages, like the prototype), and a Google Apps Script web app is the backend. The Gemini API key lives in Apps Script — it never touches the phone.

```
┌──────────┐   photo (base64)   ┌────────────────┐   image   ┌────────┐
│ index.html│ ─────────────────> │  Apps Script   │ ────────> │ Gemini │
│ (phone)   │ <───────────────── │  (Code.gs)     │ <──────── │        │
...
5. **Deploy → New deployment → Web app**
   ...
   - Copy the **`/exec`** URL.
6. Host `index.html` (or open it locally). Go to the **Setup** tab, paste the `/exec` URL, add your name, press **Test connection** — you should see "Connected ✓" and Gemini key "set".
